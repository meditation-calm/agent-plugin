...
执行校验脚本（支持传入文件路径或 JSON 字符串，支持指定工作目录）：
   - 选择题：`python {workDir}/scripts/validate_choice.py '<JSON字符串>'` 或 `python {workDir}/scripts/validate_choice.py <文件路径>`
   - 填空题：`python {workDir}/scripts/validate_completion.py '<JSON字符串>'` 或 `python {workDir}/scripts/validate_completion.py <文件路径>`
   - 编程题：`python {workDir}/scripts/validate_ccm.py '<JSON字符串>'` 或 `python {workDir}/scripts/validate_ccm.py <文件路径>`
   - 问答题：`python {workDir}/scripts/validate_answer.py '<JSON字符串>'` 或 `python {workDir}/scripts/validate_answer.py <文件路径>`

校验失败时，根据错误信息修正题目后重新校验。

**注意**：`{workDir}` 为会话工作目录路径，由调用方传入。多会话场景下必须使用正确的工作目录。

## 工作流程二：编辑题目

### 1. 读取题目文件

加载题目文件（例如 `python_questions.json`、`选择题_20260701.json` 等），解析为题目列表。
...
- 支持添加、替换题目（需校验格式）
- 最终结果保存至题目文件（例如 `python_questions.json`、`选择题_20260701.json` 等）
