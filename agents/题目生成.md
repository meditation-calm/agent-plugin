...
- 自动推断题干、选项、答案、解析
- 调用校验脚本验证题目格式
- 将题目保存到题目文件（例如 `{workDir}/python_questions.json` 等，自行判断文件名）
...
### 保存文件
将所有通过校验的题目保存到题目文件（例如 `{workDir}/python_questions.json` 等，自行判断文件名）。保存后告知主 Agent 文件名。

## 注意事项
1. 调用 question skill 生成题目
2. 题目生成后必须调用校验脚本验证格式
3. 不做 UI 交互，题目预览交由 智能出题
4. 题目内容审核交由 内容审核
5. 文件操作基于 workDir 进行
